...
int DXArchive_VER5::AddFileNameData(const TCHAR *FileName, u8 *FileNameTable)
{
	...
	// サイズをセット
	Length = static_cast<int>(_tcslen(FileName));
...
void DXArchive_VER5::KeyCreate(const char *Source, unsigned char *Key)
{
	...
	if(Source == NULL)
	{
		memset(Key, 0xaa, DXA_KEYSTR_LENGTH_VER5);
	}
	else
	{
		Len = strlen(Source);
...
NOENCODE:
		...
		else
		{
			...
			if(srcaddress + maxconbo < SrcSize)
			{
				sp2 = &sp[1];
#ifdef B64
				for(j = 1; j < maxconbo && (u64)&sp2[2] - (u64)srcp < (u64)SrcSize; j++, sp2++)
#else
...
int DXArchive_VER5::EncodeArchive(TCHAR *OutputFileName, TCHAR **FileOrDirectoryPath, int FileNum, bool Press, const char *KeyString)
{
	...
	// 各バッファを確保する
	if((NameP = (u8 *)calloc(DXA_BUFFERSIZE_VER5, sizeof(u8))) == NULL)
	{
		free(TempBuffer);
		return -1;
	}

	if((FileP = (u8 *)calloc(DXA_BUFFERSIZE_VER5, sizeof(u8))) == NULL)
	{
		free(TempBuffer);
		free(NameP);
		return -1;
	...
	if((DirP = (u8 *)calloc(DXA_BUFFERSIZE_VER5, sizeof(u8))) == NULL)
	{
		free(TempBuffer);
		free(NameP);
		free(FileP);
...
int DXArchive_VER5::DecodeArchive(TCHAR *ArchiveName, const TCHAR *OutputPath, const char *KeyString)
{
	...
	// ヘッダを解析する
	{
		...
		{
			// バージョン２以前か調べる
			memset(Key, 0xff, DXA_KEYSTR_LENGTH_VER5);
...
int	DXArchive_VER5::OpenArchiveMem(void *ArchiveImage, int ArchiveSize, const char *KeyString)
{
	...
	{
		// バージョン２以前か調べる
		memset(this->Key, 0xff, DXA_KEYSTR_LENGTH_VER5);
...
int DXArchive_VER5::OpenArchiveFileMem(const TCHAR *ArchivePath, const char *KeyString)
{
	...
	// メモリに読み込む
	{
		...
		fread(ArchiveImage, ArchiveSize, 1, pFp);
		fclose(pFp);
	}
...
ERR:
	...
	{
		free(this->HeadBuffer); this->HeadBuffer = NULL;
	}
...
int DXArchive_VER5::GetCurrentDir(TCHAR *DirPathBuffer, int BufferLength)
{
	...
	{
		return static_cast<int>(_tcslen(DirPath) + 1);
	}
	else
	{
		// ディレクトリ名をバッファに転送する
