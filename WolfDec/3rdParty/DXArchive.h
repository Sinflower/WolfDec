...
public :
	...
	DARC_FILEHEAD *GetFileInfo( const TCHAR *FilePath ) ;							// ファイルの情報を得る
...
protected :
	...
	static int DirectoryEncode(TCHAR *DirectoryName, u8 *NameP, u8 *DirP, u8 *FileP, DARC_DIRECTORY *ParentDir, SIZESAVE *Size, int DataNumber, FILE *DestP, void *TempBuffer, bool Press, unsigned char *Key ) ;	// 指定のディレクトリにあるファイルをアーカイブデータに吐き出す
